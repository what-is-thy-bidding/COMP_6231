...
public class INSE_server { 
	public static LinkedList<data_structure> inse_student_list = new LinkedList<data_structure>();//-----------------------> Contains a list of all the COMP SCI students and their data
...
//-------------------------------------------------HOME SERVER METHODS-------------------------------------	
	...
	public static void add_courses(){ //--------------------------> Addition of all courses in the server
		courses_availability fall_6110 = new courses_availability("INSE 6110", "FALL",3);
		courses_availability winter_6110 = new courses_availability("INSE 6110", "WINTER",3);
		...
		courses_availability fall_6150 = new courses_availability("INSE 6150", "FALL",3);
		courses_availability winter_6150 = new courses_availability("INSE 6150", "WINTER",3);
		courses_availability summer_6150 = new courses_availability("INSE 6150", "SUMMER",3);
	...
	public static synchronized String delete_course(String input) {//----------->ADVISOR DELETING A COURSE
		...
		while(it.hasNext()){
			...
			if(node.get_course_name().equals(list.get_course_name()) && node.get_term().equals(list.get_term())){
				drop_course(input);
				request_soen("drop SOENS " + CourseId + " " + Term);
	...
	public static synchronized String add_SOEN_course(String data){
		try{
			String [] array = data.split(" ");
	...
	public static synchronized String drop_SOEN_course(String data){
		//Increase that increase that courses availability 
	...
	public static String recieve() {
		...
		try{
			...
			while(true){
				...
				}else if((input.contains("COMPS")||input.contains("SOENS")) && input.contains("add")) {
					result=decrease_availability(input);
					sendData=result.getBytes();
				}else if((input.contains("COMPS")||input.contains("SOENS")) && input.contains("drop")){
					result=increase_availability(input);
					sendData=result.getBytes();
				}else if(input.contains("INSES") && input.contains("drop")) {
	...
	public static void main(String args[]) {
		...
		try {
			...
			Endpoint endpoint = Endpoint.publish("http://localhost:8000/inse", obj);
            recieve();
          
			//System.out.println("INFORMATION Server ready and waiting ...");		
		}
