...
import data_structures.data_structure;
import data_structures.courses_availability;;


public class COMP_server { 
	public static LinkedList<data_structure> comp_student_list = new LinkedList<data_structure>();//-----------------------> Contains a list of all the COMP SCI students and their data
...
//-------------------------------------------------HOME SERVER METHODS-------------------------------------	
	...
	public static void add_courses(){ //--------------------------> Addition of all courses in the server
		courses_availability fall_6231 = new courses_availability("COMP 6231", "FALL",3);
		courses_availability winter_6231 = new courses_availability("COMP 6231", "WINTER",3);
		...
		courses_availability winter_6651 = new courses_availability("COMP 6651", "WINTER",3);
	...
	public static String recieve() {
		...
		try{
			...
			while(true){
				...
				}if((input.contains("INSES")|| input.contains("SOENS")) && input.contains("add")) {
					result = decrease_availability(input);
					sendData=result.getBytes();
				}else if((input.contains("INSES")||input.contains("SOENS")) && input.contains("drop")) {
					result=increase_availability(input);
				}else if(input.contains("COMPS") && input.contains("drop")) {
	...
	public static void main(String args[]) {
		...
		try {
			...
			Endpoint endpoint = Endpoint.publish("http://localhost:8080/comp", obj);
            recieve();
		}
