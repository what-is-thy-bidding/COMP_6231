...
import com.web.server.COMP_server;
import com.web.server.INSE_server;
import com.web.server.SOEN_server;
...
@WebService(endpointInterface="com.web.service.WebServices")
@SOAPBinding(style=Style.RPC)

public class systemObj implements WebServices { 
	...
	public String add_course(String command) {
		...
		if(command.contains("add COMPS")){//----------------> FOR COMP STUDENTS ADDING COURSES(HOME SERVER) 
		 ...
		 if(command.contains("add INSES")) {//INSE STUDENT
			...
			return array[1]+": ("+array[2]+" "+array[3]+" "+array[4] + ") " +command;
	}
	...
	public String new_course(String command) {
		try{
			...
			String [] array = command.split(" ");
			return array[1]+": ("+array[2]+" "+array[3]+" "+array[4] + ") " +": Input is  Incorrect or Insufficient";
		}
		return command;
	}
	...
	public String delete_course(String command) {
		...
		String department = array[2];
		String course= array[2]+" "+array[3]+" "+array[4];
		if(command.contains("COMPA")) {
			if(department.contains("COMP")) {
				command=COMP_server.delete_course(command);
			}else {
				return  UserId+"("+course+ "): cannot delete that course ";
			}
	...
	public String swap_course(String enrolled, String swap) {
		...
		String [] array2 = swap.split(" ");

		String Swap_Term=array2[4];
		if(!Swap_Term.equals(Enrolled_Term)) {
			return array1[1] + ": ("+array1[2]+" "+array1[3]+" "+array1[4] +" , "+array2[2]+" "+array2[3]+" "+array2[4] + ") : THE TERMS OF THE 2 COURSES HAVE TO BE THE SAME";
		}
		...
		if(result1.contains("COURSE DELETED")) {
			...
			if(!result1.contains("Course Added") && !result1.contains("The course has already been added to the semester")) {// THE REQUESTED FOR IS NOT AVAIABLE 
				...
				add_course(enrolled);
				result="("+array2[2]+" "+array2[3]+" "+array2[4]+") "+"THE REQUESTED COURSE IS NOT AVAILABLE";
			}
		}else if(!result1.contains("COURSE DELETED")) {// NOT ENROLLED IN THAT COURSE
			result ="("+ array1[2]+" "+array1[3]+" "+array1[4]+") "+"STUDENT IS NOT ENROLLED IN THAT COURSE";
		}
	
		return array1[1] +": "+result;
	}
